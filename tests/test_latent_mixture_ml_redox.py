...
def test_latent_feature_building_and_projection(synthetic_raw_df):
    ...
    redox_input = pd.DataFrame({ID_COL: sens[ID_COL], 'axis_direct': [0.1, 0.9], 'axis_indirect': [0.7, 0.2], 'axis_network': [0.3, 0.4]})
...
def test_mixture_and_redox_workflow(synthetic_raw_df):
    ...
    net_df = pd.DataFrame({ID_COL: sens[ID_COL], 'degree': [5, 1], 'betweenness': [0.5, 0.1]})
    redox = build_redox_axes(fit_df, sens, hits, net_df=net_df)
    assert {'axis_direct', 'axis_indirect', 'axis_network', 'redox_role'}.issubset(redox.columns)

    annot = pd.DataFrame({ID_COL: sens[ID_COL], 'pathway': ['A', 'B']})
    redox_summary = summarize_redox_by_pathway(redox, annot)
    assert {'pathway', 'N', 'frac_direct_core', 'frac_indirect_responder', 'frac_network_mediator', 'frac_peripheral'}.issubset(redox_summary.columns)
