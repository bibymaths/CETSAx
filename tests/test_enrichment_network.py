...
def test_pathway_effect_and_enrichment_functions():
    ...
    assert {'pathway', 'N_proteins', 'NSS_mean', 'NSS_median', 'NSS_top25'}.issubset(summary.columns)

    hits_df = pd.DataFrame({ID_COL: metric_df[ID_COL], 'hit_class': ['strong', 'strong', 'strong', 'weak', 'weak', 'weak']})
    ora = enrich_overrepresentation(hits_df, annot, strong_labels=('strong',), min_genes=2)
    assert not ora.empty
    assert {'pathway', 'odds_ratio', 'pval', 'qval'}.issubset(ora.columns)

    cont = enrich_continuous_mannwhitney(metric_df[[ID_COL, 'NSS']], annot, score_col='NSS', min_genes=2)
