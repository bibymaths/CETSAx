...
def test_plot_protein_curve_raises_for_missing_id(synthetic_raw_df):
    ...
    with pytest.raises(ValueError):
        plot_protein_curve(
            apply_basic_qc(synthetic_raw_df), fit_df, protein_id="MISSING"
        )
...
def test_high_level_viz_helpers_render():
    path_df = pd.DataFrame(
        {
            "pathway": ["A", "B"],
            "NSS_mean": [0.9, 0.4],
            "frac_direct_core": [0.7, 0.2],
            "frac_indirect_responder": [0.2, 0.3],
            "frac_network_mediator": [0.1, 0.4],
            "frac_peripheral": [0.0, 0.1],
        }
    )
    enr_df = pd.DataFrame(
        {"pathway": ["A", "B"], "odds_ratio": [4.0, 1.5], "qval": [0.01, 0.2]}
    )
    redox_df = pd.DataFrame(
        {
            "axis_direct": [0.1, 0.9],
            "axis_indirect": [0.8, 0.2],
            "redox_role": ["mixed", "direct_core"],
        }
    )
    scores_df = pd.DataFrame(
        {"PC1": [1.0, -1.0], "PC2": [0.5, -0.5]}, index=["P1", "P2"]
    )
    ...
    assert (
        plot_redox_role_composition(path_df)[1].get_ylabel() == "Fraction of proteins"
    )
