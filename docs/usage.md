...
=== "Docker"
    ...
    Use this for a fully isolated, reproducible environment.

---

### Hardware notes

!!! tip "GPU strongly recommended for sequence modeling"
    ...
    * If no GPU is available, set `device: "cpu"` in `config.yaml` and disable or skip
...
=== "Docker"
    ...
    ```bash
    docker compose -f docker-compose.dev.yml run --rm cetsax \
        snakemake --profile workflow/profiles/local
    ```
...
!!! warning "Pipeline stops early"
    Check missing dependencies and verify `config.yaml` paths are correct and the
    input CSV exists.

!!! warning "Poor curve fits"
    Data may be noisy or non-sigmoidal. Increase QC thresholds or inspect raw curves
    before proceeding.

!!! warning "Sequence model fails"
    Likely a GPU memory issue. Reduce `esm_batch_size` or switch to `mode: "pooled"`
    in `config.yaml`.

!!! warning "Empty outputs"
